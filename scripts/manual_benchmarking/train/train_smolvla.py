# SmolVLA training with checkpoint push and true-resume support (optimizer/scheduler state).
...
FINETUNE_LR = float(_get_env("FINETUNE_LR", "5e-5"))
...
def load_pre_post_from_checkpoint(checkpoint_dir: Path, device: torch.device):
    pre = PolicyProcessorPipeline.from_pretrained(
        checkpoint_dir / "preprocessor",
...
def main():
    ...
    if RESUME:
        ...
        policy = SmolVLAPolicy.from_pretrained(str(pretrained_dir)).to(device).train()
        policy.config.device = device.type
        policy.config.use_amp = USE_AMP

        pre, post = load_pre_post_from_checkpoint(checkpoint_dir, device)
